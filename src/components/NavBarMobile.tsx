...
export default function NavBarMobile({navOpen, onClose}: MobileNavProps) {
    const showMobileNav = navOpen ? 'translate-x-0' : 'translate-x-full';
  return (
    <>
      <div onClick={onClose} className={`fixed inset-0 transform right-0 z-50 bg-black opacity-50 w-full h-screen transition-all duration-500 bg-[var(--khoa-dark-crimson)] ${showMobileNav} lg:hidden`}>
      </div>
