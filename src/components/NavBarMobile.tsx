...
export default function NavBarMobile({navOpen}: MobileNavProps) {
  ...
  return (
    <>
      ...
      <ul className={`fixed flex items-center justify-center text-white flex-col h-full transform transition-all duration-500 delay-300 w-[80%] sm:w[60%] bg-[#3a0f09] space-u-1 z-80  right-0 top-0 ${showMobileNav}`}>
        {NavLinks.map((link) => {
            return (
                <li key={link.href}>
